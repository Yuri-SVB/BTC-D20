...
Several seed tools can complete the last word for you: given all previous words, they list exactly the candidates that make the checksum valid --- for example SeedSigner's \textit{final word} flow, the SeedPicker last-word calculator, Coinplate's offline BIP39 Recoverer, or Ian Coleman's BIP39 page (used offline). For a 12-word seed there are always exactly \textbf{128 valid last words: precisely one in each of the $8\times16=128$ rows} of the table. So instead of column-by-column trial and error: roll D1 and D2 as usual, then take, among the wizard's candidates, \textbf{the one lying in your rolled section and row}. Same word, same entropy, fewer keystrokes.
...
This tutorial is by \textbf{Yuri da Silva Villas Boas} --- author of BIP-450 (Formosa) and creator of the Great Wall protocol. Official repository (sources, PDFs in 4 languages, automatic verification): \texttt{github.com/Yuri-SVB/BTC-D20} --- or scan the QR code on the side.

\begin{description}[itemsep=1pt]
 \item [Short paper:] why a quiet fix was impossible --- Kerckhoffs and the July 2026 incident: \texttt{posts/kerckhoffs-lemma-coldcard.md} in the repository
 \item [More tutorials, courses and community:] \texttt{www.loudproudandfree.com} --- and follow us on Twitter/X, Nostr and Telegram
 \item [Great Wall:] Generating a strong seed is only step one; protecting it against theft and coercion is the rest. Great Wall is our free-software protocol for coercion-resistant self-custody: \texttt{github.com/Yuri-SVB}
\end{description}
...
\editionsfooter{Rev. 2026-08 --- Extended}
