...
This last one is more tentative, and it is the strategic heart of the matter. Bitcoin's answer to physical coercion is obscurity at two levels. The individual level — low profile, deny holdings, deny harder under attack — is not the target here: that is hygiene, and rational for a holder with nothing better to migrate to. The target is the **collective** level: the tacit norm against saying plainly that a whole class of custody designs is dangerous. The class is easy to state. Whenever a scheme's protection degrades into a **feasible-but-slower path to the funds** — expiring timelocks, co-signers who can be coerced one by one, recovery and inheritance flows — coercion stops being an event and becomes a **race** that the attacker can win by patience or escalation; the [Deadly Race paper](https://github.com/Yuri-SVB/great-wall-docs/blob/main/papers/wrench-attack-threat-model/main.pdf) formalizes the race and the design bar that follows (leave no feasible path, or put it provably out of reach — nothing in between).

The hush around this is the pre-day-zero silence replayed at design-class scale, and it fails both of its own justifications. It does not withhold anything from attackers: **wrench attackers derive the race operationally** — persistence and escalation are their proof of concept, and the [public registry of physical bitcoin attacks](https://github.com/jlopp/physical-bitcoin-attacks) shows the numbers of a discovery already made. And it does not protect holders: it keeps them inside racing designs believed safe while **the exposed install base compounds** — absence of attacks against a scheme is no more evidence of coercion-resistance than five quiet years were evidence of a healthy RNG. Disclosure of a *design class* exposes no individual, so Kerckhoffs' conclusion transfers whole: the custody worth having remains safe when the attacker knows everything — holdings, amounts, and the exact protocol guarding them. That bar, and which designs meet it by which route, is the program of the [Great Wall](https://github.com/Yuri-SVB) line of work.

## Sources

- [Villas Boas — The Deadly Race: a wrench-attack threat model](https://github.com/Yuri-SVB/great-wall-docs/blob/main/papers/wrench-attack-threat-model/main.pdf)
